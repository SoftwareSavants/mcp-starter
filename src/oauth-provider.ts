...
/**
 * In-memory client store. In production, replace with a database-backed store.
 */
...
export function createOAuthProvider() {
  ...
  requiredEnv("OAUTH_CLIENT_SECRET"); // validated — used by upstream token exchange
  ...
  return new ProxyOAuthServerProvider({
    ...
    verifyAccessToken: async (token: string): Promise<AuthInfo> => {
      // Verify the token with the upstream service.
      // This example calls the upstream's userinfo / introspection endpoint.
      // Replace with your service's token verification logic.
