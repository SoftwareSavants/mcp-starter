...
export async function startHttp(
  ...
  // --- OAuth endpoints (/authorize, /token, /register, etc.) ---
  ...
  // --- MCP over HTTP ---
  // Each session gets its own transport and server instance so the tool
  // handlers use the authenticated user's token for API calls.
  ...
  app.post("/mcp", bearerAuth, async (req, res) => {
    ...
    if (!transport) {
      // New session — create transport + server wired to this user's token.
